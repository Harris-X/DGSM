...
def _multiway_variants_for_base_key(base_key: str) -> Tuple[str, ...]:
    """给定 base 的参数名，生成 donor 侧可能存在的 multiway 变体键。

    适配 mplug-owl2 的多路命名：
      - input_layernorm.weight -> input_layernorm.multiway.{0,1}.weight
      - post_attention_layernorm.weight -> post_attention_layernorm.multiway.{0,1}.weight
      - self_attn.k_proj.weight -> self_attn.k_proj.multiway.{0,1}.weight
      - self_attn.v_proj.weight -> self_attn.v_proj.multiway.{0,1}.weight
    其他权重（q_proj/o_proj/mlp/*）保持单一路径。
    """
    variants = []
    if '.input_layernorm.weight' in base_key:
        variants.append(base_key.replace('.input_layernorm.weight', '.input_layernorm.multiway.0.weight'))
        variants.append(base_key.replace('.input_layernorm.weight', '.input_layernorm.multiway.1.weight'))
    if '.post_attention_layernorm.weight' in base_key:
        variants.append(base_key.replace('.post_attention_layernorm.weight', '.post_attention_layernorm.multiway.0.weight'))
        variants.append(base_key.replace('.post_attention_layernorm.weight', '.post_attention_layernorm.multiway.1.weight'))
    if '.self_attn.k_proj.weight' in base_key:
        variants.append(base_key.replace('.self_attn.k_proj.weight', '.self_attn.k_proj.multiway.0.weight'))
        variants.append(base_key.replace('.self_attn.k_proj.weight', '.self_attn.k_proj.multiway.1.weight'))
    if '.self_attn.v_proj.weight' in base_key:
        variants.append(base_key.replace('.self_attn.v_proj.weight', '.self_attn.v_proj.multiway.0.weight'))
        variants.append(base_key.replace('.self_attn.v_proj.weight', '.self_attn.v_proj.multiway.1.weight'))
    return tuple(variants)


def _get_donor_weight(donor: Dict[str, torch.Tensor], base_key: str, combine: str = 'mean') -> torch.Tensor | None:
    """在 donor 权重中查找与 base_key 对应的权重。

    查找顺序：
      1) 直接匹配 base_key
      2) 针对 mplug-owl2 等，尝试插入 multiway.{0,1} 生成两个候选键并组合（mean/sum）
      3) 尝试 language_model. 前缀的上述两步（为兼容 onevision/qwenvl）
      4) 退回 _alt_donor_key 的其它容错规则
    """
    # 1) 直接命中
    if base_key in donor:
        return donor[base_key]
    
    # 2) multiway 组合
    candidates = list(_multiway_variants_for_base_key(base_key))

    found = [donor[k] for k in candidates if k in donor]
    if len(found) == 2:
        return (found[0] + found[1]) / 2.0 if combine == 'mean' else (found[0] + found[1])
    if len(found) == 1:
        return found[0]

    # 3) language_model. 前缀再尝试一次
    lm_key = f'language_model.{base_key}'
    if lm_key in donor:
        return donor[lm_key]
    lm_candidates = [f'language_model.{k}' for k in _multiway_variants_for_base_key(base_key)]
    found_lm = [donor[k] for k in lm_candidates if k in donor]
    if len(found_lm) == 2:
        return (found_lm[0] + found_lm[1]) / 2.0 if combine == 'mean' else (found_lm[0] + found_lm[1])
    if len(found_lm) == 1:
        return found_lm[0]

    # 4) 其它容错
    alt = _alt_donor_key(donor, base_key)
    return donor[alt] if alt is not None else None
...
def dgsm_merge(args: argparse.Namespace):
    ...
    # 全局对齐景观统计：mu_d / sigma_d（用于 CAGA 门控）
    d_list = []
    for _mod_k, _blk in modules_info.items():
        try:
            if isinstance(_blk, dict):
                if 'psi_A' in _blk and isinstance(_blk['psi_A'], torch.Tensor) and _blk['psi_A'].numel() >= 3:
                    d_list.append(float(_blk['psi_A'][2]))
                elif 'gwd_cost' in _blk:
                    d_list.append(float(_blk['gwd_cost']))
        except Exception:
            pass
    if len(d_list) == 0:
        ...
        mu_d = float(_stats.mean(d_list))
        if len(d_list) >= 2:
            # 使用总体标准差（population stdev）
            try:
                sigma_d = float(_stats.pstdev(d_list))
            except Exception:
                sigma_d = float(torch.tensor(d_list).std(unbiased=False))
        else:
            sigma_d = 0.0
    print(f"[CAGA] Global mu_d={mu_d:.6f}, sigma_d={sigma_d:.6f} (N={len(d_list)})")
    ...
    for k in tqdm(list(weights_A.keys()), desc='DGSM Merge'):
        ...
        if k.endswith('.weight') and weights_A[k].ndim==2:
            ...
            W_B=_get_donor_weight(weights_B, k, combine=str(getattr(args,'multiway_combine','mean')))
            if W_B is None:
                missing_in_B += 1
            ...
            if blk is not None:
                ...
                if has_tefm:
                    ...
                    # Mapping (CAGA): 计算层级 TOS_l = cosine(psi*, psi*') * g_l * exp(-beta * d)
                    # 1) 选择用于余弦的向量（psi 或 psi'）
                    cosine_mode = str(getattr(args, 'mapping_cosine_mode', 'psi_prime'))
                    if cosine_mode == 'psi':
                        vA = psi_A
                        vB = psi_B
                    else:
                        ...
                        vB = psi_B[:2]
                    # 2) 计算余弦相似（层级）
                    nA = float(torch.norm(vA).clamp_min(EPS))
                    nB = float(torch.norm(vB).clamp_min(EPS))
                    cos_layer = float(torch.dot(vA, vB) / (nA * nB))
                    cos_layer = max(0.0, min(1.0, cos_layer))  # ReLU 到 [0,1]
                    # 3) 计算门控 g_l = exp(-max(0, (d - mu)/ (sigma + eps)))
                    eps_map = float(getattr(args, 'entropy_eps', EPS))
                    z = 0.0
                    if sigma_d > 0.0:
                        z = (bar_d - mu_d) / (sigma_d + eps_map)
                    g_l = math.exp(-max(0.0, z))
                    # 4) 距离衰减项
                    mapping_beta = float(getattr(args, 'mapping_beta', 0.1))
                    dist_decay = math.exp(-mapping_beta * max(bar_d, 0.0))
                    tos_scalar = cos_layer * g_l * dist_decay
                    tos_scalar = float(max(0.0, min(1.0, tos_scalar)))

                    # 5) 将 TOS_l 作为 alignment 的标量门控（避免标量缩放后再归一导致抵消）
                    pi = blk['pi'].float()
                    pi_eff = M_tensor @ pi if M_tensor is not None else pi
                    alignment = pi_eff @ S_B
                    if alignment.numel() > 0 and alignment.max() > 0:
                        alignment = alignment / alignment.max().clamp_min(EPS)
                        alignment = alignment * tos_scalar
    ...
    meta=dict(base_model=args.base_model, donor_model=args.donor_model, stage2=args.stage2,
              ...
              mapping_beta=float(getattr(args, 'mapping_beta', 0.1)),
              mapping_cosine_mode=str(getattr(args, 'mapping_cosine_mode', 'psi_prime')),
              caga_mu_d=mu_d, caga_sigma_d=sigma_d)
    base_dir=osp.basename(args.base_model.rstrip(os.sep))
    out_root=osp.join(args.output_dir, base_dir, 'dgsm_merged'); os.makedirs(out_root, exist_ok=True)
...
def parse_args():
    ...
    ap.add_argument('--mapping-cosine-mode', type=str, default='psi_prime', choices=['psi','psi_prime'],
                    help="TOS 余弦相似度所用的编码：'psi' 使用 [\bar{s},\bar{h},\bar{d}], 'psi_prime' 使用 [\bar{s},\bar{h}]")
    ap.add_argument('--multiway-combine', type=str, default='mean', choices=['mean','sum'], help='当 donor 出现 multiway.{0,1} 两分支时如何组合为单一路径')
