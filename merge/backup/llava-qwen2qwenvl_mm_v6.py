...
class AGIDPMMerger:
    ...
    def stage3_project_and_merge(self):
        ...
        # Compute global bar_tau per parameter type
        global_tau = defaultdict(list)
        for key in tqdm(non_conflict_masks.keys(), desc="Collecting tau for coordination"):
            if not need_merge(key): continue
            key_in_c = key.replace("model.language_model.", "model.")
            if not (key_in_c in weights_B and key_in_c in weights_C): continue
            tau_B = (weights_B[key_in_c] - weights_C[key_in_c]).float()
            param_type = key.split('.')[-2]  # e.g., 'q_proj'
            global_tau[param_type].append(tau_B)

        bar_tau = {}
        for ptype, taus in global_tau.items():
            bar_tau[ptype] = torch.mean(torch.stack(taus), dim=0)
        ...
        for key, mask_B in tqdm(non_conflict_masks.items(), desc="执行合并"):
            ...
            bar = bar_tau[param_type].to(self.device)
            ...
            tau_B_disjoint = c_i * tau_B * mask_B.to(self.device)
            
            d_i_norm_sq = torch.sum(d_i * d_i)
            if d_i_norm_sq > epsilon:
                # 投影操作
            ...
            u_shared = activations_B[module_name]['output'].to(self.device) - activations_A[module_name]['output'].to(self.device)
            v_proj = tau_B_proj @ d_i
            v_ortho = tau_B_ortho @ d_i
            V = torch.stack([v_proj, v_ortho], dim=1)  # D x 2
            VT = V.t()  # 2 x D
            gram = VT @ V  # 2 x 2
            gram += torch.eye(2, device=self.device) * epsilon
            inv_gram = torch.inverse(gram)
            lambda0 = inv_gram @ (VT @ u_shared)

            V_lambda = V @ lambda0
            Y_merged = activations_A[module_name]['output'].to(self.device) + V_lambda

            T = torch.norm(V_lambda)
            if T > epsilon:
                grad_T = VT @ (V_lambda / T)
            else:
                grad_T = torch.zeros(2, device=self.device)

            dot = Y_merged @ activations_A[module_name]['output'].to(self.device)
            norm_m = torch.norm(Y_merged)
            norm_a = torch.norm(activations_A[module_name]['output'].to(self.device))
            S = dot / (norm_m * norm_a + epsilon)

            grad_S_temp = activations_A[module_name]['output'].to(self.device) / (norm_m * norm_a + epsilon) - (dot * Y_merged / (norm_m**3 * norm_a + epsilon))
            grad_S = VT @ grad_S_temp

            lambda_star = lambda0 + self.args.beta1 * grad_T + self.args.beta2 * grad_S

            # 最终合并
            W_star = W_A.to(self.device).float() + lambda_star[0] * tau_B_proj + lambda_star[1] * tau_B_ortho
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mode', type=str, default="agidpm-v1", help="为本次合并配置命名。")
    ...
    parser.add_argument('--beta1', type=float, default=0.1, help="Beta1 for transfer regularizer.")
    parser.add_argument('--beta2', type=float, default=0.05, help="Beta2 for retention regularizer.")
