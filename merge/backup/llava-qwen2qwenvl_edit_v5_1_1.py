...
# =================================================================================
# ASAM 3.0 - 优化后的 _cache_activations_for_model 函数
# 核心更改：
# 1. 钩子注册逻辑更精确，只在叶子模块上注册。
# 2. 钩子函数本身更简化，只负责捕获输入和输出。
# 3. 新增一个数据重组步骤，将从叶子模块捕获的激活整理成阶段二期望的格式。
# =================================================================================

    def _cache_activations_for_model(self, model_name, model_path, cache_path, capture_inputs):
        """阶段一的核心函数（优化版）：为单个模型执行前向传播并缓存激活。"""
        ...
        captured_activations_flat = defaultdict(lambda: defaultdict(list))
        hooks = []

        # --- 优化点 1: 简化的钩子函数 ---
        # 它不再需要知道模块类型，只负责捕获输入和输出。
        def get_hook(module_name):
            def hook_fn(module, input_tensors, output_tensors):
                # 健壮地处理各种输入和输出格式
                input_tensor = input_tensors[0] if isinstance(input_tensors, tuple) else input_tensors
                output_tensor = output_tensors[0] if isinstance(output_tensors, tuple) else output_tensors

                if capture_inputs and isinstance(input_tensor, torch.Tensor):
                    captured_activations_flat[module_name]['input'].append(input_tensor.detach().cpu())
                
                if isinstance(output_tensor, torch.Tensor):
                    captured_activations_flat[module_name]['output'].append(output_tensor.detach().cpu())
        ...
        for name, module in model_to_hook.named_modules():
            # 使用一个虚拟的权重名称来检查模块是否为我们的目标
            key_for_check = "model." + name + ".weight"
            if self._should_merge(key_for_check):
                hooks.append(module.register_forward_hook(get_hook(name)))

        print(f"在 {model_name} 中注册了 {len(hooks)} 个钩子到叶子模块。")

        # 准备探针数据集和前向传播 (此部分逻辑不变)
        ...
        # 平均所有捕获的激活
        averaged_activations_flat = {}
        for name, data in captured_activations_flat.items():
            averaged_activations_flat[name] = {k: torch.mean(torch.cat(v, dim=0).float(), dim=0) for k, v in data.items() if v}

        # --- 优化点 3: 数据重组 ---
        # 将从叶子模块捕获的扁平数据，重组为阶段二期望的、按父模块组织的格式。
        final_averaged_activations = defaultdict(dict)
        for module_name, avg_activations in averaged_activations_flat.items():
            if "mlp" in module_name:
                # MLP模块直接复制
                final_averaged_activations[module_name] = avg_activations
            elif "self_attn" in module_name:
                attn_block_name = ".".join(module_name.split('.')[:-1])
                
                if module_name.endswith(".q_proj"):
                    final_averaged_activations[attn_block_name]['q'] = avg_activations['output']
                    if 'input' in avg_activations:
                        final_averaged_activations[attn_block_name]['input'] = avg_activations['input']
                elif module_name.endswith(".k_proj"):
                    final_averaged_activations[attn_block_name]['k'] = avg_activations['output']
                elif module_name.endswith(".v_proj"):
                    final_averaged_activations[attn_block_name]['v'] = avg_activations['output']
                elif module_name.endswith(".o_proj"):
                    # o_proj的输出就是整个attention块的输出
                    final_averaged_activations[attn_block_name]['output'] = avg_activations['output']

        torch.save(dict(final_averaged_activations), cache_path)
        print(f"{model_name} 的激活已缓存至: {cache_path}")

        del model, tokenizer, captured_activations_flat, averaged_activations_flat, final_averaged_activations
        gc.collect()
