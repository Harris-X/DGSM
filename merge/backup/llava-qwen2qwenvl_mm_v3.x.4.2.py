...
# --- 核心实现类 ---
class PISAMPlusMerger:
    def __init__(self, args, device):
        self.args = args
        self.device = device
        self.output_dir = os.path.join("merged_models", f"pisam-plus-{args.mode}")
    ...
    def stage2_regularized_saliency_analysis(self):
        ...
        print("\n--- [阶段二: PISA-M+ 正则化重要性评分] ---")
        mask_cache_path = os.path.join(self.cache_dir, f"pisam-plus_mask_r{self.args.top_k_ratio}_alpha{self.args.alpha}.pt")
    ...
    def stage3_post_alignment_conflict_resolved_fusion(self):
        """阶段三：【PISA-M+】执行后对齐冲突消解与融合。"""
        print("\n--- [阶段三: PISA-M+ 后对齐冲突消解与融合] ---")
        ...
        pbar = tqdm(unified_masks.items(), desc="【PISA-M+】执行对齐与融合")
        for key, M_i in pbar:
            ...
            tau_B_full = (W_B - W_C).to(self.device)
            tau_B_aligned = s * (tau_B_full @ R)

            # 步骤 2: 计算对齐后的差值向量
            tau_diff = tau_B_aligned - tau_A

            # 步骤 3: 后对齐冲突消解
            conflict_mask = torch.sign(tau_A) != torch.sign(tau_B_aligned)
            non_conflict_mask = ~conflict_mask
            delta_W = tau_diff * non_conflict_mask

            # 步骤 4: 计算最终合并权重
            W_star = W_A.to(self.device) + self.args.lambda_ * delta_W
    ...
    def run_pipeline(self):
        ...
        self.stage3_post_alignment_conflict_resolved_fusion()

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="使用PISA-M+进行最终的、理论完备且高效的模型合并。")
    ...
    parser.add_argument('--mode', type=str, default="pisam-plus-final", help="为本次合并配置命名。")
    ...
    parser.add_argument('--lambda_', type=float, default=0.5, dest='lambda_', help="【阶段三】无冲突知识增量的吸收率，唯一核心可调超参数。")
    ...
    merger = PISAMPlusMerger(args, device)
    merger.run_pipeline()