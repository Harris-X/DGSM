...
# --- 核心实现类 ---
# NEW: 引入全新的 FA_AGIDPMMerger 类来实现新方法
class FA_AGIDPMMerger:
    ...
    def stage2_analyze_neurons_and_get_masks(self):
        ...
        print("\n--- [阶段二: 费雪增强神经元分析 (FA-AGIDPM)] ---")
        mask_cache_path = os.path.join(self.cache_dir, f"fa_non_conflict_masks_r{self.args.top_k_ratio}.pt")
        ...
        for key in tqdm(weights_A.keys(), desc="分析神经元"):
            ...
            try:
                ...
                # 【新】步骤 2: 计算近似费雪信息 F'
                fisher_approx_A = g_approx_A.square()
                fisher_approx_B = g_approx_B.square()

                # 【新】步骤 3: 计算费雪增强重要性分数 I_FA
                W_A, W_B, W_C = weights_A[key], weights_B[key], weights_C[key]
                importance_A = (W_A.float() * g_approx_A).abs() * fisher_approx_A
                importance_B = (W_B.float() * g_approx_B).abs() * fisher_approx_B

                # 步骤 4: 选举与分离 (逻辑不变，但基于新的重要性分数)
                # 处理可能出现的 NaN 或 Inf 值
                importance_A[torch.isinf(importance_A)] = 0
                importance_B[torch.isinf(importance_B)] = 0
                
                k = int(importance_A.numel() * self.args.top_k_ratio)
                if k == 0: continue

                mask_A = importance_A >= torch.topk(importance_A.flatten(), k=k)[0].min()
                mask_B = importance_B >= torch.topk(importance_B.flatten(), k=k)[0].min()
                ...
                non_conflict_masks[key] = final_mask_B.cpu() # 存到CPU以节省显存
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mode', type=str, default="fa-agidpm-default", help="为本次合并配置命名。")
    ...
    parser.add_argument('--lambda_ortho', type=float, default=0.6, help="正交（无关）分量的系数。")
    ...
    merger = FA_AGIDPMMerger(args, device)
    merger.run_pipeline()