...
def _normalize_cost(cost: float, cost_scale: Optional[float]) -> float:
    """Map GW cost to [0, 1] range for localization/fusion heuristics."""
    if cost_scale is None or cost_scale <= EPS:
        # fallback: smooth, monotonically increasing squashing
        return float(math.tanh(max(0.0, cost)))
    return float(min(1.0, max(0.0, cost / max(cost_scale, EPS))))


def _relative_stats(SA: torch.Tensor, SB: torch.Tensor, gwd_cost: float, cost_scale: Optional[float]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, float]:
    """Compute RGSP-relative singular statistics."""
    delta = SB - SA
    p_rel = _softmax(delta.abs(), dim=-1)
    bar_s = (p_rel * delta).sum()
    bar_h = -(p_rel * (p_rel.clamp_min(EPS).log())).sum()
    bar_d = _normalize_cost(gwd_cost, cost_scale)
    psi_rel = torch.stack([
        bar_s,
        bar_h,
        torch.tensor(bar_d, dtype=bar_s.dtype, device=bar_s.device)
    ])
    return delta, p_rel, psi_rel, bar_d
...
def _invalid_transport(pi: torch.Tensor, cost_val: float) -> bool:
    finite_mask = torch.isfinite(pi)
    sum_pi = float(pi[finite_mask].sum().item()) if finite_mask.any() else 0.0
...
def stage2(args: argparse.Namespace):
    ...
    out_mod={}; costs=[]; lambdas=[]; pot_fallback_count=0
    ...
    for name in it:
        ...
        if pot_enabled and _HAVE_POT:
            ...
            pi_pot, gwd_cost_pot = _pot_pi(CA, CB, method=method, eps=eps, max_iter=max_it, verbose=bool(args.verbose))
            if _invalid_transport(pi_pot, gwd_cost_pot):
                if args.verbose:
                    print(f"[DGSM-POT][Fallback] {name}: invalid transport (eps={eps}, method={method}), switching to torch GW approx")
            else:
                pi, gwd_cost = pi_pot, gwd_cost_pot
                pot_success = True
        if not pot_success:
        ...
        delta_S, p_rel, psi_rel, bar_d_norm = _relative_stats(SA, SB, gwd_cost, args.cost_scale)
        rec = {
            ...
            'p_rel': p_rel.cpu(),
            'psi_rel': psi_rel.cpu(),
            'bar_d': torch.tensor(bar_d_norm)
        }
        if M_final is not None:
            rec['M'] = M_final.cpu()
        if pot_enabled and not pot_success:
            pot_fallback_count += 1
            rec['pot_fallback'] = torch.tensor(1)
    ...
    meta = {
        ...
        'pot_fallback_modules': pot_fallback_count,
