...
def _compute_psi(S:torch.Tensor, gwd_cost:float, penalty:float=0.0)->torch.Tensor:
    p=_softmax(S)
    bar_s=(p*S).sum()
    bar_h=-(p*(p.clamp_min(EPS).log())).sum()
    bar_d=torch.tensor(float(gwd_cost + penalty))
...
def _normalize_transport(pi: torch.Tensor, iters: int = 5) -> torch.Tensor:
    if pi.ndim != 2:
        return pi
    r, c = pi.shape
    if r == 0 or c == 0:
...
def stage2(args: argparse.Namespace):
    ...
    out_mod={}; costs=[]; lambdas=[]; penalties=[]; bar_vals=[]
    ...
    for name in it:
        ...
        VA_blk = blkA.get('V'); VB_blk = blkB.get('V')
        if VA_blk is None or VB_blk is None:
            raise KeyError(f"[Stage-2] Module {name} 缺少 V 子空间，请先使用更新后的 Stage-1 重新提取子空间。")
        VA = VA_blk.float().to(device)
        VB = VB_blk.float().to(device)
        ...
        if args.dynamic_steps>0:
            ...
            dyn_iter = range(args.dynamic_steps)
            if not getattr(args, 'no_progress', False):
                dyn_iter = tqdm(dyn_iter, leave=False, desc=f"Dyn {name}")
        ...
        eye_r = torch.eye(r, device=device, dtype=pi.dtype)
        K = VB.T @ VA
        pi_corr = torch.relu(pi @ K)
        pi_corr = _normalize_transport(pi_corr)
        corr_penalty = float(torch.linalg.norm(K - eye_r).item())
        bar_d_val = float(gwd_cost + corr_penalty)

        psi_A = _compute_psi(SA, gwd_cost, corr_penalty).cpu()
        psi_B = _compute_psi(SB, gwd_cost, corr_penalty).cpu()
        rec = {
            ...
            'corr_penalty': torch.tensor(corr_penalty),
            'bar_d': torch.tensor(bar_d_val)
        }
        ...
        if args.verbose:
            ...
            print(f"[DGSM-{src}] {dyn} {name}: cost={gwd_cost:.6f} bar_d={bar_d_val:.6f} r={r} {lam_str}")
    meta = {
        'subs_a': args.subs_a,
        'subs_b': args.subs_b,
    ...
    'corr_penalty_mean': float(sum(penalties)/len(penalties)) if penalties else None,
    'bar_d_mean': float(sum(bar_vals)/len(bar_vals)) if bar_vals else None,
    ...
    if penalties:
        print(f"  corr_penalty(mean)={meta['corr_penalty_mean']:.4f}")
    if bar_vals:
        print(f"  bar_d(mean)={meta['bar_d_mean']:.4f}")
