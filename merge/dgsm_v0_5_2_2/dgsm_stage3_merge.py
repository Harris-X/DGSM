        ...
        if k.endswith('.weight') and weights_A[k].ndim==2:
            ...
            if blk is not None:
                ...
                if has_tefm:
                    ...
                    # Mapping (v0_5_2_2): TOS 基于 FSI 差的指数门控
                    # FSI = bar_s / (bar_h + eps)
                    if psi_A.numel() >= 2:
                        bar_sA = float(psi_A[0]); bar_hA = float(psi_A[1])
                    else:
                        pA = _softmax_tensor(S_A); bar_sA = float((pA * S_A).sum()); bar_hA = float(-(pA * pA.clamp_min(EPS).log()).sum())
                    if psi_B.numel() >= 2:
                        bar_sB = float(psi_B[0]); bar_hB = float(psi_B[1])
                    else:
                        pB = _softmax_tensor(S_B); bar_sB = float((pB * S_B).sum()); bar_hB = float(-(pB * pB.clamp_min(EPS).log()).sum())
                    fsi_A = bar_sA / max(float(args.entropy_eps), (bar_hA + float(args.entropy_eps)))
                    fsi_B = bar_sB / max(float(args.entropy_eps), (bar_hB + float(args.entropy_eps)))
                    mapping_gamma = float(getattr(args, 'mapping_gamma', 0.8))
                    tos_scalar = math.exp(-mapping_gamma * abs(fsi_A - fsi_B))
                    pi = blk['pi'].float()
                    # 标量门控：直接缩放 π*；(Normalize 对标量无效，这里保留简单缩放)
                    pi_adj = pi * float(tos_scalar)
                    ...
                    if alignment.numel() > 0 and alignment.max() > 0:
                        # 仍做幅度归一，随后用 TOS 标量再次门控，确保 FSI 影响保留
                        alignment = alignment / alignment.max().clamp_min(EPS)
                        alignment = alignment * float(tos_scalar)
...
def parse_args():
    ...
    # v0_5_2_2: 映射改为 FSI 差指数门控，γ 控制敏感度
    ap.add_argument('--mapping-gamma', type=float, default=0.8, help='FSI 差的敏感度 γ (TOS=exp(-γ|FSI_A-FSI_B|))')
